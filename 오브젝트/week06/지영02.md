...
🔺 Money 타입을 만들 때 내부를 long 으로 할지, BigDecimal 로 할지는?
...
🔺 메서드를 호출한다(call)가 아니라 메시지를 전송한다(send a message)가 더 적절하다? 
...
```
🔺 DiscountPolicy의 인스턴스를 생성할 필요없기 때문에 추상 클래스(abstract class)로 구현했다.
